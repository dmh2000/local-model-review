...
class StdRandom:
    ...
    def geometric(cls, p):
        if p <= 0 or p > 1:
            raise ValueError("probability p must be between 0.0 and 1.0: " + str(p))
        return math.ceil(math.log(random.random()) / math.log(1.0 - p))
    ...
    @classmethod
    def discrete(cls, probabilities):
